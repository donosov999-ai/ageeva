/* Форма заявки на курс: клик по тарифу → модалка (имя+контакт) → valya_lead_add (база + письмо Вале) → «спасибо» + Telegram.
...
(function () {
  ...
  function build() {
    ...
    modal.innerHTML = '<div class="capply" role="dialog" aria-modal="true">' +
      ...
      '<label for="capplyContact" style="margin-top:12px">Telegram, телефон или email</label><input id="capplyContact" type="text" placeholder="@username, +7… или email" autocomplete="off">' +
  ...
  function submit(e) {
    ...
    var contact = (document.getElementById('capplyContact').value || '').trim();
    var msg = document.getElementById('capplyMsg');
    if (contact.length < 3) { msg.textContent = 'Оставь контакт — Telegram, телефон или email, иначе Вале некуда ответить.'; msg.className = 'capply-msg bad'; return; }
    ...
    fetch(SB_URL + '/rest/v1/rpc/valya_lead_add', {
      ...
      .then(function () {
        ...
        box.innerHTML = '<button class="capply-x" type="button" aria-label="Закрыть">×</button>' +
          ...
          '<p>Заявка у Валентины — она свяжется с тобой в ближайшее время, подтвердит место и пришлёт ссылку на оплату.</p>' +
          '<p style="color:var(--muted);font-size:.95rem;margin-top:14px">Хочешь — напиши ей сразу, так быстрее:</p>' +
          '<a href="' + TG + '" target="_blank" rel="noopener" class="btn btn-primary btn-lg" data-goal="course_apply_tg" style="width:100%">Написать в Telegram →</a>';
